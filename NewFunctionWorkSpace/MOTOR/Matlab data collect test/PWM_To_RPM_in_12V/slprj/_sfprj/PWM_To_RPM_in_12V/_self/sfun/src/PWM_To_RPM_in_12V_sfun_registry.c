...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_PWM_To_RPM_in_12V_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_PWM_To_RPM_in_12V_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_PWM_To_RPM_in_12V_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "PWM_To_RPM_in_12V") == 0) {
    const mxArray *newRhs[3] = { NULL, NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_PWM_To_RPM_in_12V_autoinheritance_info(nlhs,plhs,3,newRhs);
  }
...
{
  extern unsigned int sf_PWM_To_RPM_in_12V_third_party_uses_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  char machineName[128];
  if (nrhs < 4) {
  ...
  if (strcmp(machineName, "PWM_To_RPM_in_12V") == 0) {
    ...
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_PWM_To_RPM_in_12V_third_party_uses_info(nlhs,plhs,3,newRhs);
  }
...
unsigned int sf_process_get_updateBuildInfo_args_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_PWM_To_RPM_in_12V_updateBuildInfo_args_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  char machineName[128];
  if (nrhs < 4) {
  ...
  if (strcmp(machineName, "PWM_To_RPM_in_12V") == 0) {
    ...
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_PWM_To_RPM_in_12V_updateBuildInfo_args_info(nlhs,plhs,3,newRhs);
  }
...
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_PWM_To_RPM_in_12V_get_eml_resolved_functions_info( int
  ...
  if (strcmp(machineName, "PWM_To_RPM_in_12V") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_PWM_To_RPM_in_12V_get_eml_resolved_functions_info(nlhs,plhs,2,
      newRhs);
  }
...
extern unsigned int sf_PWM_To_RPM_in_12V_method_dispatcher(SimStruct *S,
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"PWM_To_RPM_in_12V")) {
    return(sf_PWM_To_RPM_in_12V_method_dispatcher(simstructPtr,chartFileNumber,
...
extern void PWM_To_RPM_in_12V_initializer(void);
extern void PWM_To_RPM_in_12V_register_exported_symbols(SimStruct* S);
extern void PWM_To_RPM_in_12V_debug_initialize(struct SfDebugInstanceStruct*);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  PWM_To_RPM_in_12V_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfGlobalMdlStartCallCounts == 1) {
    ...
    if (!simModeIsRTWGen) {
      PWM_To_RPM_in_12V_debug_initialize(sfGlobalDebugInstanceStruct);
    }

    PWM_To_RPM_in_12V_initializer();
  }
